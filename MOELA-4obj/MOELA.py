...
def main(app, max_time, seed_number, init_random):
    ...
    with open('weights/sirui_weight4.csv', newline='') as f:
        reader = csv.reader(f)
        weight = list(reader)
    for i in range(len(weight)):
        weight[i] = [float(j) for j in weight[i]]
    with open('weights/sirui_weight4_n.csv', newline='') as f:
        reader = csv.reader(f)
    ...
    while elas[-1] < max_time:
        ...
        with open(method+'/seed'+str(seed_number)+'_init'+str(init_random)+'_'+method+'_errors_'+app+'4.csv', 'a+') as f:
...
if __name__ == '__main__':
    ...
    with open(method+'/seed'+str(seed_number)+'_init'+str(init_random)+'_'+method+'_errors_'+app+'4.csv', 'w') as f:
