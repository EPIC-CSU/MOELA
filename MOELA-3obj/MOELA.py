...
def main(app, max_time, seed_number, init_random):
    ...
    with open('weights/sirui_weight3.csv', newline='') as f:
        reader = csv.reader(f)
        weight = list(reader)
    for i in range(len(weight)):
        weight[i] = [float(j) for j in weight[i]]
    with open('weights/sirui_weight3_n.csv', newline='') as f:
        reader = csv.reader(f)
    ...
    mesh = [mesh_mean, mesh_dev, mesh_lat]
    ref = np.array(calc_params(ori_nodes, ori_link, traffic))[0:3]
    ...
    min_objs = (min_objs[0], 0.0, min_objs[2])
    print("min objectives:", min_objs)
    print("max objectives:", max_objs)
    scaler = MinMaxScaler((0, 100))
    print(scaler.fit([min_objs, max_objs]))
    minmax_mesh = scaler.transform([(mesh_mean, mesh_dev, mesh_lat)])[0].tolist()
    ...
    while elas[-1] < max_time:
        ...
        with open(method+'/seed'+str(seed_number)+'_init'+str(init_random)+'_'+method+'_errors_'+app+'3.csv', 'a+') as f:
...
if __name__ == '__main__':
    ...
    with open(method+'/seed'+str(seed_number)+'_init'+str(init_random)+'_'+method+'_errors_'+app+'3.csv', 'w') as f:
